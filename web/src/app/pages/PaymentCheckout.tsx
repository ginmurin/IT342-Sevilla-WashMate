...
import { ArrowLeft, Lock, AlertCircle, CreditCard, Loader2, Mail } from "lucide-react";
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-slate-100">
              ...
              <div className="flex gap-1.5">
                {["VISA", "MC", "JCB"].map((b) => (
                  <span key={b} className="px-2 py-0.5 bg-slate-100 rounded text-xs font-bold text-slate-600">{b}</span>
                ))}
              </div>
            </CardHeader>
            <CardContent className="pt-5">
              {error && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-start gap-2">
                  ...
                  <p className="text-xs text-blue-800">
                    Your card details are encrypted and processed securely by PayMongo. We never store your card information.
                  </p>
                </div>
