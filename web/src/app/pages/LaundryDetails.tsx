...
import { Settings, WashingMachine, FoldHorizontal, Zap, Wind, Check, AlertCircle } from "lucide-react";
...
export default function LaundryDetails() {
  ...
  const lineItems = SERVICES.filter((s) => selected.includes(s.type)).map((s) => ({
    ...s,
    subtotal: s.price * localWeight,
  }));

  const estimatedPrice = lineItems.reduce((sum, li) => sum + li.subtotal, 0);
  ...
  const handleNext = () => {
    ...
    setOrderData({ weight: localWeight, estimatedPrice });
    nextStep();
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
              <Card className="border-slate-200 shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg">Weight & Quantity</CardTitle>
                </CardHeader>
                        ...
                        return (
                          <li key={li.type} className="flex items-center justify-between text-sm">
                            <span className="flex items-center gap-2 text-teal-800">
                              ...
                              {li.name}
                              <span className="text-teal-600 text-xs">× {localWeight} {li.unit}</span>
                            </span>
