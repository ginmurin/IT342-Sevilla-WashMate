...
import { Lock, AlertCircle, Loader2, CheckCircle2, Crown, Sparkles } from 'lucide-react';
import { motion } from 'motion/react';
import { useSubscription } from "@/features/subscriptions/SubscriptionContext";

/**
 * Subscription Upgrade Checkout Page
          ...
          <CardContent className="pt-8 pb-8 space-y-6">
            {error ? (
              ...
              <>
                ...
                <button
                  onClick={() => window.location.href = '/subscription/upgrade-review'}
              ...
              <>
                <div className="flex justify-center">
                  ...
                  >
                    <Loader2 className={`w-8 h-8 ${isPremium ? 'text-amber-600' : 'text-teal-600'}`} />
                  </motion.div>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-semibold text-slate-900">{upgradeData.newPlan}</span>
                    </div>
                  </div>
                  <div className="flex justify-between text-sm">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    >
                      <CheckCircle2 className={`w-4 h-4 ${isPremium ? 'text-amber-600' : 'text-teal-600'}`} />
                      <span>{step}</span>
                    </motion.div>
                  ))}
                </div>
